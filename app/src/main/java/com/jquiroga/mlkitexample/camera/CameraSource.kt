...
/**
 * Administra la cámara y permite actualizaciones de la interfaz de usuario (p. Ej., Superposición
 * de gráficos adicionales). Esto recibe fotogramas de vista previa de la cámara a una velocidad especificada,
 * envía esos fotogramas al detector tan rápido como puede procesar.
 *
 * Esta fuente de cámara hace un gran esfuerzo para administrar el procesamiento en los cuadros de vista previa
 * lo más rápido posible, al tiempo que minimiza el retraso. Como tal, los cuadros pueden caerse si el detector
 * no puede mantener el ritmo de los cuadros generados por la cámara.
 */
...
class CameraSource(private val graphicOverlay: GraphicOverlay) {
    ...
    /** Devuelve el tamaño de vista previa que está actualmente en uso por la cámara subyacente.*/
    internal var previewSize: Size? = null
        private set

    /**
     * Subproceso dedicado y ejecutable asociado para llamar al detector con marcos, a medida que
     * los marcos están disponibles desde la cámara.
     */
    ...
    /**
     * Mapa para convertir entre una matriz de bytes, recibida de la cámara, y su búfer de bytes asociado.
     * Usamos buffers de bytes internamente porque esta es una manera más eficiente de llamar al código
     * nativo más adelante (evita una copia potencial).
     *
     * Nota: ** usa IdentityHashMap aquí en lugar de HashMap porque el comportamiento de los métodos igual,
     * hashCode y toString de una matriz es inútil e inesperado. IdentityHashMap aplica la verificación
     * de identidad ('==') en las claves.
     */
    private val bytesToByteBuffer = IdentityHashMap<ByteArray, ByteBuffer>()
    private val context: Context = graphicOverlay.context

    /**
     * Abre la cámara y comienza a enviar fotogramas de vista previa al detector subyacente. El soporte
     * de superficie suministrado se usa para la vista previa para que los marcos se puedan mostrar al usuario.
     *
     * @param surfaceHolder el soporte de superficie para usar para los marcos de vista previa.
     * @throws IOException si el soporte de superficie suministrado no se puede utilizar como visualización de vista previa.
     */
    @Synchronized
    ...
    /**
     * Cierra la cámara y deja de enviar fotogramas al detector de fotogramas subyacente.
     *
     *
     * Esta fuente de cámara se puede reiniciar nuevamente llamando a [.start].
     *
     *
     * Call [.release] en lugar de apagar completamente esta fuente de cámara y liberar el
          * recursos del detector subyacente.
     */
    @Synchronized
    internal fun stop() {
    ...
    /** Detiene la cámara y libera los recursos de la cámara y el detector subyacente.  */
    fun release() {
